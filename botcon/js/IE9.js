...
(function(window, document) {
	...
	IE7.compat = 9;
	var appVersion = IE7.appVersion = navigator.appVersion
			.match(/MSIE (\d\.\d)/)[1] - 0;

	if (/ie7_off/.test(top.location.search) || appVersion < 5.5
			|| appVersion >= IE7.compat)
		return;
	...
	var _BASE = /\bbase\b/;
	var _HIDDEN = [ "constructor", "toString" ]; // only override these when
													// prototyping
	...
	// A collection of regular expressions and their associated replacement
	// values.
			...
			.extend(
					{
						...
						add : function(expression, replacement) {
							...
							if (!this[HASH + expression])
								this[KEYS].push(String(expression));
							return this[ITEMS][HASH + expression] = new RegGrp.Item(
									expression, replacement, this);
						},

						compile : function(recompile) {
							if (recompile || !this[COMPILED]) {
								this[COMPILED] = new RegExp(this,
						...
						exec : function(string) {
							...
							if (result) {
								...
								while ((item = items[HASH + patterns[i++]])) {
									...
									if (result[offset]) { // do we have a
															// result?
										if (item.replacement === 0) {
											return group.exec(string);
										} else {
											var args = result.slice(offset,
													next), j = args.length;
											while (--j)
												args[j] = args[j] || ""; // some
																			// platforms
												...
												while (--i)
													args[i] = arguments[i]
															|| ""; // some
																	...
																	// non-matching
																	// sub-expressions
												// Loop through the RegGrp
												// items.
												while ((item = items[HASH
														+ patterns[i++]])) {
													var next = offset
															+ item.length + 1;
													if (args[offset]) { // do we
																		// have
																		// a
																		// result?
					...
					{
						IGNORE : null, // a null replacement value means that
									...
									constructor : function(source, replacement,
										...
										if (dictionary
											...
											if (REGGRP_DICT_ENTRY.test(source)) {
												var entry = dictionary[ITEMS][HASH
														+ source.slice(2, -1)];
										...
										// Does the expression use
										// sub-expression lookups?
										if (typeof replacement == "string"
												&& REGGRP_LOOKUP
														.test(replacement)) {
											if (REGGRP_LOOKUP_SIMPLE
													.test(replacement)) { // A
																			// simple
																			// lookup?
																			// (e.g.
																			// "$2").
												// Store the index (used for
												// fast retrieval of matched
												// strings).
						...
						count : function(expression) {
							return (String(expression).replace(
									REGGRP_ESCAPE_COUNT, "")
									.match(REGGRP_PAREN) || "").length;
						}
					});

	var Dictionary = RegGrp
			.extend({
				parse : function(phrase) {
					// Prevent sub-expressions in dictionary entries from
					// capturing.
					var entries = this[ITEMS];
					return phrase.replace(REGGRP_DICT_ENTRIES, function(match,
							entry) {
				...
				add : function(expression, replacement) {
					...
					// The result is the original replacement recursively parsed
					// by this dictionary.
					var nonCapturing = replacement.replace(REGGRP_LOOKUPS,
							_nonCapture);
					...
					item._length = realLength || item.length; // underlying
																// number of
																// sub-groups
					return item;
				},
		...
		if (object && source) {
			...
			for (key in source)
				if (typeof proto[key] == "undefined") {
					...
					if (object[key] && typeof value == "function"
							&& _BASE.test(value)) {
						_override(object, key, value);
					} else {
						object[key] = value;
	...
	function encodeString(string) {
		...
		strings[index] = string.slice(1, -1).replace(UNESCAPE, "$1").replace(
				SINGLE_QUOTES, "\\'");
		return "'" + index + "'";
	}
	;
	...
	function register(handler, element, condition) { // -@DRE
		...
		if (!handler.elements)
			handler.elements = {};
		if (condition)
			handler.elements[element.uniqueID] = element;
		else
	...
	function getDefinedStyle(element, propertyName) {
		return element.currentStyle[$IE7 + propertyName]
	...
	var NEXT_SIBLING = "(e.nextSibling&&IE7._getElementSibling(e,'next'))", PREVIOUS_SIBLING = NEXT_SIBLING
			.replace(/next/g, "previous"), IS_ELEMENT = "e.nodeName>'@'", IF_ELEMENT = "if("
			+ IS_ELEMENT + "){";
			...
			.extend({ // single instance
				...
				init : function() {
					...
					if (this.pseudoClasses)
						pseudoClasses.push(this.pseudoClasses);
					var dynamicPseudoClasses = this.dynamicPseudoClasses
							.toString();
					if (dynamicPseudoClasses)
						pseudoClasses.push(dynamicPseudoClasses);
					pseudoClasses = pseudoClasses.join("|");
					var unknown = appVersion < 7 ? [ "[>+~\\[(]|([:.])[\\w-]+\\1" ]
							: [ CLASS ];
					if (pseudoClasses)
						unknown.push(":(" + pseudoClasses + ")");
					this.UNKNOWN = new RegExp(unknown.join("|") || NONE, "i");
					var complex = appVersion < 7 ? [ "\\[[^\\]]+\\]|[^\\s(\\[]+\\s*[+~]" ]
							: [ CLASS ];
					var complexRule = complex.concat();
					if (pseudoClasses)
						complexRule.push(":(" + pseudoClasses + ")");
					Rule.COMPLEX = new RegExp(complexRule.join("|") || NONE,
							"ig");
					if (this.pseudoClasses)
						complex.push(":(" + this.pseudoClasses + ")");
					DynamicRule.COMPLEX = new RegExp(complex.join("|") || NONE,
							"i");
					dynamicPseudoClasses = "not\\(:"
							+ dynamicPseudoClasses.split("|").join(
									"\\)|not\\(:") + "\\)|"
							+ dynamicPseudoClasses;
					DynamicRule.MATCH = new RegExp(
							dynamicPseudoClasses ? "(.*?):("
									+ dynamicPseudoClasses + ")(.*)" : NONE,
							"i");
				...
				addRecalc : function(propertyName, test, handler, replacement) {
					// recalcs occur whenever the document is refreshed using
					// document.recalc()
					propertyName = propertyName.source || propertyName;
					test = new RegExp("([{;\\s])" + propertyName + "\\s*:\\s*"
							+ test + "[^;}]*");
					...
					this.addFix(test, function(match) {
						...
						return (replacement ? replacement : match) + ";ie7-"
								+ match.slice(1) + ";ie7_recalc" + id + ":1";
					});
				...
				getText : function(styleSheet, path) {
					// Internet Explorer will trash unknown selectors (it
					// converts them to "UNKNOWN").
					// So we must reload external style sheets (internal style
					// sheets can have their text
					...
					if (httpRequest)
						cssText = loadFile(styleSheet.href, path) || cssText;
					return cssText;
				},
				...
				recalc : function() {
					...
					// only calculate screen fixes. print fixes don't show up
					// anyway
					...
					for (i = start; rule = rules[i]; i++) {
						...
						// search for the "ie7_recalc" flag (there may be more
						// than one)
						if (calcs = cssText.match(RECALCS)) {
							...
							// to each element
							if (elements.length)
								for (j = 0; j < calcs.length; j++) {
									// get the matching flag (e.g. ie7_recalc3)
									id = calcs[j];
									// extract the numeric id from the end of
									// the flag
									// and use it to index the collection of
									// recalc
									// functions
									calc = IE7.CSS.recalcs[id.slice(10)][2];
									for (k = 0; (element = elements[k]); k++) {
										// apply the fix
										if (element.currentStyle[id])
											calc(element, cssText);
									}
								}
						}
					}
	...
	var StyleSheet = Base
			.extend({
				...
				createRule : function(selector, cssText) {
					var match;
					if (PseudoElement
							&& (match = selector.match(PseudoElement.MATCH))) {
						return new PseudoElement(match[1], match[2], cssText);
					} else if (match = selector.match(DynamicRule.MATCH)) {
						if (!HYPERLINK.test(match[0])
								|| DynamicRule.COMPLEX.test(match[0])) {
							return new DynamicRule(selector, match[1],
									match[2], match[3], cssText);
						}
					} else {
				...
				getText : function() {
					...
					function getCSSText(styleSheet, path, media, level) {
						...
						if (media === "all" || media === self.media) {
							// IE only allows importing style sheets three
							// levels deep.
							// it will crash if you try to access a level below
							// this
							try {
							...
							if (level < 3 && canAcess) {
								...
								for ( var i = 0, imported; i < styleSheet.imports.length; i++) {
									...
									var href = styleSheet._href
											|| styleSheet.href;
									imported._href = hrefs[i].replace(
											TRIM_IMPORTS, "");
									// call this function recursively to get all
									// imported style sheets
									cssText += getCSSText(imported, getPath(
											href, path), media, level + 1);
								}
							}
							// retrieve inline style or load an external style
							// sheet
							cssText += encode(styleSheet.href ? loadStyleSheet(
									styleSheet, path)
									: styleSheet.owningElement._cssText);
					...
					for ( var i = 0; i < styleSheets.length; i++) {
						...
						if (!styleSheet.disabled && !styleSheet.ie7)
							this.cssText += getCSSText(styleSheet);
					}
					...
					function loadStyleSheet(styleSheet, path) {
						...
						if (fileCache[url])
							return "";
						// Load from source
						fileCache[url] = styleSheet.disabled ? "" : fixUrls(
								IE7.CSS.getText(styleSheet, path), getPath(
										href, path));
					...
					// Fix CSS paths.
					// We're lumping all css text into one big style sheet so
					// relative
					// paths have to be fixed. This is necessary anyway because
					// of other
					// Internet Explorer bugs.
					function fixUrls(cssText, pathname) {
						// hack & slash
						return cssText.replace(URL, "$1"
								+ pathname.slice(0,
										pathname.lastIndexOf("/") + 1) + "$2");
					}
					;
				},
				...
				parse : function() {
					...
					var declarations = "";
					this.cssText = cssText.replace(
							/@charset[^;]+;|@font\-face[^\}]+\}/g, function(
									match) {
								declarations += match + "\n";
								return "";
							});
					...
					while ((rule = RULE.exec(this.cssText))) {
						var cssText = rule[2];
						if (cssText) {
							var fixDescendants = appVersion < 7
									&& cssText.indexOf("AlphaImageLoader") !== -1;
							...
							for ( var i = 0; selector = selectors[i]; i++) {
								...
								selectors[i] = isUnknown ? this.createRule(
										selector, cssText) : selector + "{"
										+ cssText + "}";
								if (fixDescendants)
									selectors[i] += this.createRule(selector
											+ ">*", "position:relative");
							}
	...
	var Rule = IE7.Rule = Base.extend({
		constructor : function(selector, cssText) {
			...
			this.selector = (pseudoElement ? pseudoElement[1] : selector)
					|| "*";
			this.selectorText = this.parse(this.selector)
					+ (pseudoElement ? pseudoElement[2] : "");
		...
		parse : function(selector) {
			...
			// much as we can..
			var simple = selector.replace(Rule.CHILD, " ").replace(
					Rule.COMPLEX, "");
			if (appVersion < 7)
				simple = simple.replace(Rule.MULTI, "");
			var tags = match(simple, Rule.TAGS).length
					- match(selector, Rule.TAGS).length;
			var classes = match(simple, Rule.CLASSES).length
					- match(selector, Rule.CLASSES).length + 1;
	...
	var DynamicRule = Rule
			.extend({
				// properties
				constructor : function(selector, attach, dynamicPseudoClass,
						target, cssText) {
					this.negated = dynamicPseudoClass.indexOf("not") === 0;
					if (this.negated)
						dynamicPseudoClass = dynamicPseudoClass.slice(5, -1);
				...
				recalc : function() {
					...
					for ( var i = 0; attach = attaches[i]; i++) {
						...
						var target = this.target ? cssQuery(this.target, attach)
								: [ attach ];
						// attach event handlers for dynamic pseudo-classes
						if (target.length)
							this.dynamicPseudoClass.apply(attach, target, this);
					}
				}
			});
	...
	var ATTR = {
		"=" : "%1==='%2'", // "[@%1='%2']"
		"~=" : "(' '+%1+' ').indexOf(' %2 ')!==-1", // "[contains(concat('
													// ',@%1,' '),' %2 ')]",
		"|=" : "%1==='%2'||%1.indexOf('%2-')===0", // "[@%1='%2' or
													// starts-with(@%1,'%2-')]",
	...
	if (appVersion < 7) {
		...
		IE7.HTML.addRecalc("label",
				function(label) {
					if (!label.htmlFor) {
						var firstChildControl = cssQuery("input,textarea",
								label, true);
	...
	(function() {
		...
		layout.boxSizing = function(element) {
			if (!element.currentStyle.hasLayout) {
				...
				// when an element acquires "layout", margins no longer collapse
				// correctly
		...
		function collapseMargin(element, type) {
			if (!element.runtimeStyle[type]) {
				...
				if (parentElement
						&& parentElement.currentStyle.hasLayout
						&& !IE7._getElementSibling(element,
								isTopMargin ? "previous" : "next"))
					return;
				var child = element[isTopMargin ? "firstChild" : "lastChild"];
				if (child && child.nodeName < "@")
					child = IE7._getElementSibling(child, isTopMargin ? "next"
							: "previous");
				if (child && child.currentStyle.styleFloat === "none"
						&& child.currentStyle.hasLayout) {
					...
					if (margin < 0 || childMargin < 0) {
						...
						element.runtimeStyle[type] = Math.max(childMargin,
								margin);
					}
		...
		var _fixWidth = function(HEIGHT) {
			apply.Width = function(element) {
				if (!PERCENT.test(element.currentStyle.width))
					_fixWidth(element);
				if (HEIGHT)
					collapseMargins(element);
			};
			...
			function _fixWidth(element, value) {
				if (!element.runtimeStyle.fixedWidth) {
					...
					element.runtimeStyle.fixedWidth = UNIT.test(value) ? Math
							.max(0, getFixedWidth(element, value))
							+ "px" : value;
					setOverrideStyle(element, "width",
							element.runtimeStyle.fixedWidth);
				}
			}
			;
			...
			var getFixedWidth = function(element, value) {
				...
				var adjustment = 0;
				if (MSIE5 && !borderBox)
					adjustment += getBorderWidth(element)
							+ getWidth(element, "padding");
			...
			function getWidth(element, type) {
				return getPixelWidth(element, element.currentStyle[type
						+ "Left"])
						+ getPixelWidth(element, element.currentStyle[type
								+ "Right"]);
			}
			;
			...
			layout.minWidth = function(element) {
				...
				// #if (element.currentStyle.minHeight === "auto")
				// element.runtimeStyle.minHeight = 0;
				if (element.currentStyle["min-width"] != null) {
					element.style.minWidth = element.currentStyle["min-width"];
				}
				if (register(arguments.callee, element,
						element.currentStyle.minWidth !== "none")) {
			...
			function resizeWidth(element) {
				...
				if (element.currentStyle.minWidth !== "none"
						&& width < getFixedWidth(element,
			...
			function fixRight(element) {
				if (register(fixRight, element, /^(fixed|absolute)$/
						.test(element.currentStyle.position)
			...
			function resizeRight(element) {
				var left = getPixelWidth(element, element.runtimeStyle._left
						|| element.currentStyle.left);
				var width = layoutWidth(element)
						- getPixelWidth(element, element.currentStyle.right)
						- left - getWidth(element, "margin");
				if (parseInt(element.runtimeStyle.width) === width)
					return;
			...
			addResize(function() {
				...
				for (i in elements) {
					...
					var fixedWidth = (parseInt(element.runtimeStyle.width) === getFixedWidth(
							element, element.currentStyle.minWidth));
					if (wider && fixedWidth)
						element.runtimeStyle.width = "";
					if (wider == fixedWidth)
						resizeWidth(element);
				...
				for (i in elements) {
					var element = elements[i];
					var fixedWidth = (parseInt(element.runtimeStyle.width) === getFixedWidth(
							element, element.currentStyle.maxWidth));
					if (!wider && fixedWidth)
						element.runtimeStyle.width = "";
					if (wider !== fixedWidth)
						resizeWidth(element);
							...
							.addRecalc(
									...
									function(element) {
										element.runtimeStyle.pixelHeight = parseInt(layoutWidth(element)
												* element.currentStyle["ie7-height"]
														.slice(0, -1) / 100);
									});
			}
		};
	...
	var PNG_FILTER = "progid:" + ALPHA_IMAGE_LOADER
	...
	if (appVersion < 7) {
		...
		// replace background(-image): url(..) .. with background(-image): ..
		// ;filter: ..;
		IE7.CSS.addFix(
				/background(-image)?\s*:\s*([^};]*)?url\(([^\)]+)\)([^;}]*)?/,
				function(match, $1, $2, url, $4) {
					url = getString(url);
					return PNG.test(url) ? "filter:"
							+ format(PNG_FILTER, url,
									$4.indexOf("no-repeat") === -1 ? "scale"
		...
		// list-style-image
		IE7.CSS.addRecalc(/list\-style(\-image)?/, "[^};]*url", function(
				element) {
	...
	function addFilter(element, sizingMethod, src) {
		...
		if (filter) {
			...
			element.runtimeStyle.filter = format(PNG_FILTER,
					src || element.src, sizingMethod || "scale");
	...
	(function() {
		...
		IE7.CSS.addRecalc("background(-attachment)?", "[^};]*fixed",
				_backgroundFixed);
		...
		function _fixBackground() {
			// this is required by both position:fixed and
			// background-attachment:fixed.
			// it is necessary for the document to also have a fixed background
			// image.
		...
		function _isFixed(element) {
			return element ? isFixed(element)
					|| _isFixed(element.parentElement) : false;
		}
		;

		function _setExpression(element, propertyName, expression) {
			setTimeout("document.all." + element.uniqueID
		...
		function _backgroundFixed(element) {
			if (register(_backgroundFixed, element,
					element.currentStyle.backgroundAttachment === "fixed"
		...
		function _backgroundPosition(element) {
			...
			var parentElement = element.canHaveChildren ? element
		...
		function _foregroundPosition(element, recalc) {
			...
			if (!element.runtimeStyle.autoLeft
				...
				var left = viewport.clientWidth
						- util.getPixelWidth(element,
				...
				if (_isFixed(element.offsetParent))
					element.runtimeStyle.pixelLeft += left;
				else
					element.runtimeStyle.shiftLeft = left;
			}
			if (!element.runtimeStyle.fixedWidth)
				util.clipWidth(element);
			if (!element.runtimeStyle.fixedHeight)
				util.clipHeight(element);
		}
		;
		...
		var _horizontal = function(util) {
			util.bgLeft = function(element) {
				...
				if (!_isFixed(element)) {
					_setExpression(element, "backgroundPositionX",
			...
			util.setOffsetLeft = function(element) {
				var propertyName = _isFixed(element) ? "backgroundPositionX"
						: "offsetLeft";
				element.runtimeStyle[propertyName] = util.getOffsetLeft(
						element, element.style.backgroundPositionX)
			...
			util.clipWidth = function(element) {
				...
				if (element.currentStyle.width !== "auto") {
					var rect = element.getBoundingClientRect();
					var width = element.offsetWidth - viewport.clientWidth
							+ rect.left - 2;
					if (width >= 0) {
			...
			util.positionLeft = function(element, recalc) {
				// if the element's width is in % units then it must be
				// recalculated
				...
				if (element.runtimeStyle.fixWidth) {
					element.runtimeStyle.width = util.getPixelWidth(element,
							element.runtimeStyle.fixWidth);
				}
				// if (recalc) {
				// // if the element is fixed on the right then no need to
				// recalculate
				...
				// }
				// reset the element's "left" value and get it's natural
				// position
				...
				// if the element is contained by another fixed element then
				// there is no need to
				...
				if (!recalc && !_isFixed(element.offsetParent)) {
					...
					_setExpression(element, "pixelLeft",
			...
			util.getScreenLeft = function(element) { // thanks to kevin
														// newman (captainn)
				var screenLeft = element.offsetLeft, nested = 1;
				if (element.runtimeStyle.autoLeft) {
					screenLeft = viewport.clientWidth
							- element.offsetWidth
							- util.getPixelWidth(element,
									element.currentStyle.right);
				}
				...
				if (element.currentStyle.marginLeft !== "auto") {
					screenLeft -= util.getPixelWidth(element,
			...
			util.getPixelWidth = function(element, value) {
				return PERCENT.test(value) ? parseInt(parseFloat(value) / 100
						* viewport.clientWidth) : getPixelValue(element, value);
			};
		};
	 ...
	 * ---------------------------------------------------------------------
	 * 
	 * This module alters the structure of the document. It may adversely affect
	 * other CSS rules. Be warned.
	 * 
	 * ---------------------------------------------------------------------
	 */
		...
		IE7.CSS
				.addRecalc(
						...
						function(element) {
							...
							// don't do this again
							if (element.parentNode.ie7_wrapped)
								return;

							// if max-height is applied, makes sure it gets
							// applied first
							if (IE7.Layout
									&& element.currentStyle["max-height"] !== "auto") {
	...
	function ie7Quirks() {
		var FONT_SIZES = "xx-small,x-small,small,medium,large,x-large,xx-large"
				.split(",");
		for ( var i = 0; i < FONT_SIZES.length; i++) {
			FONT_SIZES[FONT_SIZES[i]] = FONT_SIZES[i - 1] || "0.67em";
		}

		IE7.CSS.addFix(/(font(-size)?\s*:\s*)([\w.-]+)/, function(match, label,
				size, value) {
		...
		function getFontScale(element) {
			...
			while (element != body) {
				...
				if (fontSize) {
					if (EM.test(fontSize))
						scale *= parseFloat(fontSize);
					else if (PERCENT.test(fontSize))
						scale *= (parseFloat(fontSize) / 100);
					else if (EX.test(fontSize))
						scale *= (parseFloat(fontSize) / 2);
					else {
		...
		function fixMargin(element) {
			...
			var margin = parent.offsetWidth - element.offsetWidth
		...
		IE7.CSS.addRecalc("margin(-left|-right)?", "[^};]*auto", function(
				element) {
			if (register(fixMargin, element, element.parentElement
	...
	if (appVersion < 8) {
		...
		function parseInherited(cssText) {
			return cssText.replace(
					new RegExp("([{;\\s])(" + inheritedProperties.join("|")
							+ ")\\s*:\\s*([^;}]+)", "g"), "$1$2:$3;ie7-$2:$3");
		}
		;
						...
						function(element, cssText) {
							if (element.parentElement) {
								...
								for ( var i = 0; i < inherited.length; i++) {
									var propertyName = inherited[i].replace(
											STRIP_IE7_FLAGS, "");
									if (element.currentStyle["ie7-"
											+ propertyName] === "inherit") {
							...
							inheritedProperties.push(rescape(match.slice(1)
									.split(":")[0]));
							return match;
						});
		...
		var PseudoElement = IE7.PseudoElement = Rule
				.extend(
						{
							constructor : function(selector, position, cssText) {
								...
								var content = cssText
										.match(PseudoElement.CONTENT), match, entity;
								if (content) {
									content = content[1];
									...
									for ( var i = 0; (entity = match[i]); i++) {
										match[i] = /^attr/.test(entity) ? {
											attr : entity.slice(5, -1)
												...
												: entity.charAt(0) === "'" ? getString(entity)
														: decode(entity);
									}
									content = match;
								}
								...
								if (generated) {
									...
									for ( var j = 0; j < content.length; j++) {
										if (typeof content[j] == "object") {
											content[j] = target
													.getAttribute(content[j].attr);
										}
									}
									content = content.join("");
									var url = content.match(URL);
									var cssText = "overflow:hidden;"
											+ generated.cssText.replace(/'/g,
													'"');
									var position = POSITION_MAP[this.position
											+ Number(target.canHaveChildren)];
									var id = 'ie7_pseudo'
											+ PseudoElement.count++;
									target.insertAdjacentHTML(position, format(
											PseudoElement.ANON, this.className,
											id, cssText, url ? "" : content));
									if (url) {
										var src = getString(url[1]);
										var pseudoElement = document
												.getElementById(id);
										pseudoElement.src = src;
										addFilter(pseudoElement, "crop");
										var targetIsFloated = target.currentStyle.styleFloat !== "none";
										if (pseudoElement.currentStyle.display === "inline"
												|| targetIsFloated) {
											if (appVersion < 7
													&& targetIsFloated
											...
											image.onload = function() {
												...
												pseudoElement.style.pixelHeight = Math
														.max(
																this.height,
	...
	IE7.HTML.addRecalc("button,input", function(button) {
		...
		if (button.type === "submit") {
			addEventHandler(button, "onclick", function() {
				...
				setTimeout("document.all." + button.uniqueID
	...
	IE7.HTML.addRecalc("form", function(form) {
		addEventHandler(form, "onsubmit", function() {
			for ( var element, i = 0; element = form[i]; i++) {
				if (UNSUCCESSFUL.test(element.type) && !element.disabled
	...
	if (appVersion < 8) {
		IE7.CSS.addRecalc("border-spacing", NUMERIC,
				function(element) {
					if (element.currentStyle.borderCollapse !== "collapse") {
						element.cellSpacing = getPixelValue(element,
								element.currentStyle["ie7-border-spacing"]
										.split(" ")[0]);
					}
				});
	...
	var NOT_NEXT_BY_TYPE = "!IE7._getElementSiblingByType(e,'next')&&", NOT_PREVIOUS_BY_TYPE = NOT_NEXT_BY_TYPE
	...
	var Indeterminate = new DynamicPseudoClass("indeterminate", function(
			element) {
			...
			extend(
					{
						":nth(-last)?-(?:child|(of-type))\\((<#nth_arg>)\\)(<#filter>)?" : function(
								match, last, ofType, args, filters) { // :nth-
																		// pseudo
																		// classes
							args = args.replace(SPACES, "");

							var index = "IE7._indexOf(e," + !!last + ","
									+ !!ofType + ")";
...
mark{background:yellow}";
	...
	var cssQuery = (function() {
		...
		// This is not a selector engine in the strictest sense. So it's best to
		// silently error.
		function cssQuery(selector, context, single) {
			...
			try {
				return selectQuery.create(selector, useContext)(context,
						single ? null : [], ref);
			} catch (ex) {
				return single ? null : [];
			}
		...
		IE7._getAttribute = function(element, name) {
			...
			if (element[name] && typeof element[name] == "boolean")
				return name.toLowerCase();
			if ((specified && USE_IFLAG.test(name)) || (!attribute && MSIE5)
					|| name === "value" || name === "type") {
				return element.getAttribute(name, 2);
			}
			if (name === "style")
				return element.style.cssText.toLowerCase() || null;
		...
		// Convert the list of strings to a hash, mapping the lowercase name to
		// the camelCase name.
		extend(_ATTRIBUTES, combine(names.toLowerCase().split(","), names
				.split(",")));
		...
		IE7._byId = function(document, id) {
			...
			for ( var i = 0; i < result.length; i++) {
				if (IE7._getAttribute(result[i], "id") === id)
					return result[i];
			}
			return null;
		};
		...
		function normalize(selector) {
			selector = normalizer.parse(selector.replace(HEX_ESCAPE, "\\x$1"))
					.replace(UNESCAPE, "$1").replace(IMPLIED_ASTERISK, "$1*$2");
		...
		function getConstants(selector) {
			...
			if (ROOT.test(selector))
				constants += ",R=d.documentElement";
			if (TARGET.test(selector))
				constants += ",H=d.location;H=H&&H.hash.replace('#','')";
			if (constants || selector.indexOf("#") !== -1) {
				constants = ",t=c.nodeType,d=t===9?c:c.ownerDocument||(c.document||c).parentWindow.document"
						+ constants;
			}
		...
		var COMBINATOR = {
			...
			">" : ".parentElement;if(e){",
			"+" : ";while((e=e.previousSibling)&&!(" + IS_ELEMENT
					+ "))continue;if(e){",
		...
		MATCHER = new CSSSelectorParser(
				{
					"(?:(<#selector>)(<#combinator>))?(<#tag>)(<#filter>)?$" : function(
							match, before, combinator, tag, filters) {
						var group = "";
						if (tag !== "*") {
		...
		var selectById = new CSSSelectorParser({
			"^(<#tag>)#(<#ident>)(<#filter>)?( [^,]*)?$" : function(match,
					tagName, id, filters, after) {
			...
			"^([^#,]+)#(<#ident>)(<#filter>)?$" : function(match, before, id,
					filters) {
		...
		var selectQuery = new CSSSelectorParser({
			"<#grammar>" : function(match, selector, remainingSelectors) {
				...
				var groups = this.groups, tagName = groups[0][TAG_NAME]; // first
																			// tag
																			// name
				...
				for ( var i = 0; group = groups[i]; i++) {
					MATCHER.index = 0;
					if (tagName !== "*")
						group[TAG_NAME] = "*"; // we are already filtering by
												// tagName
					group = group.join("");
					if (group === " *") { // select all
						matcher = store;
		...
		selectQuery.create = function(selector) {
			if (!cache[selector]) {
				...
				if (selector.indexOf("#") !== -1) {
					...
					if (byId) {
						block = "if(t===1||t===11|!c.getElementById){" + block
								+ "}else{" + byId + "}";
					}
				}
				...
				cache[selector] = new Function(
						"return function(c,r,s){var i=0,k=0,e0;" + block
								+ "return r}")();
			}
	...
	(function() {
		...
		if (typeof IE7_PNG_SUFFIX == "object") {
			...
			PNG = new RegExp(rescape(window.IE7_PNG_SUFFIX || "-trans.png")
					+ "(\\?.*)?$", "i");
		}
