...
var Zepto = (function() {
	var undefined, key, $, classList, emptyArray = [], slice = emptyArray.slice, filter = emptyArray.filter, document = window.document, elementDisplay = {}, classCache = {}, getComputedStyle = document.defaultView.getComputedStyle, cssNumber = {
		...
		'z-index' : 1,
		'zoom' : 1
	}, fragmentRE = /^\s*<(\w+|!)[^>]*>/, tagExpanderRE = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig, rootNodeRE = /^(?:body|html)$/i,
	...
	}, readyRE = /complete|loaded|interactive/, classSelectorRE = /^\.([\w-]+)$/, idSelectorRE = /^#([\w-]*)$/, tagSelectorRE = /^[\w-]+$/, class2type = {}, toString = class2type.toString, zepto = {}, camelize, uniq, tempParent = document
			.createElement('div')
	...
	$.fn = {
		...
		html : function(html) {
			return html === undefined ? (this.length > 0
					? this[0].innerHTML
		...
		text : function(text) {
			return text === undefined ? (this.length > 0
					? this[0].textContent
		...
		val : function(value) {
			return (value === undefined) ? (this[0] && (this[0].multiple
		...
		css : function(property, value) {
			if (arguments.length < 2 && typeof property == 'string')
				return this[0]
						&& (this[0].style[camelize(property)] || getComputedStyle(
								this[0], '').getPropertyValue(property))
	...
	['width', 'height'].forEach(function(dimension) {
				$.fn[dimension] = function(value) {
					var offset, el = this[0], Dimension = dimension.replace(
							/./, function(m) {
					...
					if (value === undefined)
						return isWindow(el)
								? el['inner' + Dimension]
								: isDocument(el) ? el.documentElement['offset'
										+ Dimension] : (offset = this.offset())
										&& offset[dimension]
					else
						return this.each(function(idx) {
									el = $(this)
...
(function($) {
	...
	function eventCapture(handler, captureSetting) {
		return handler.del && (handler.e == 'focus' || handler.e == 'blur')
				|| !!captureSetting
	}
					...
					findHandlers(element, event, fn, selector).forEach(
							function(handler) {
								...
								element.removeEventListener(
										realEvent(handler.e), handler.proxy,
										eventCapture(handler, capture))
							})
				})
	}
	...
	$.fn.delegate = function(selector, event, callback) {
		return this.each(function(i, element) {
					add(element, event, callback, selector, function(fn) {
								return function(e) {
									var evt, match = $(e.target).closest(
	...
	$.fn.on = function(event, selector, callback) {
		return !selector || $.isFunction(selector) ? this.bind(event, selector
						|| callback) : this.delegate(selector, event, callback)
	}
	$.fn.off = function(event, selector, callback) {
		return !selector || $.isFunction(selector) ? this.unbind(event,
				selector || callback) : this.undelegate(selector, event,
				callback)
	}
	...
	$.fn.triggerHandler = function(event, data) {
		...
		this.each(function(i, element) {
					e = createProxy(typeof event == 'string'
							? $.Event(event)
							: event)
					e.data = data
					e.target = element
...
(function($) {
	...
	$.ajaxJSONP = function(options) {
		...
		var callbackName = 'jsonp' + (++jsonpID), script = document
				.createElement('script'), cleanup = function() {
			clearTimeout(abortTimeout)
			$(script).remove()
			delete window[callbackName]
		}, abort = function(type) {
			cleanup()
			// In case of manual abort or timeout, keep an empty function as
			// callback
			// so that the SCRIPT tag that eventually loads won't result in an
			// error.
	...
	$.ajax = function(options) {
		...
		var mime = settings.accepts[dataType], baseHeaders = {}, protocol = /^([\w-]+:)\/\//
				.test(settings.url) ? RegExp.$1 : window.location.protocol, xhr = settings
				.xhr(), abortTimeout
	...
	function parseArguments(url, data, success, dataType) {
		...
		return {
			...
			success : !hasData ? data : $.isFunction(success)
					? success
					: undefined,
	...
	$.fn.animate = function(properties, duration, ease, callback) {
		...
		if (duration)
			duration = (typeof duration == 'number'
					? duration
					: ($.fx.speeds[duration] || $.fx.speeds._default))
					/ 1000
...
(function($) {
		...
		$(document.body).bind('touchstart', function(e) {
			...
			if ((touch.x2 && Math.abs(touch.x1 - touch.x2) > 30)
				...
				swipeTimeout = setTimeout(function() {
							...
							touch.el.trigger('swipe'
									+ (swipeDirection(touch.x1, touch.x2,
											touch.y1, touch.y2)))
							touch = {}
						}, 0)
			...
			// normal tap
				...
				if (deltaX < 30 && deltaY < 30) {
					// delay by one tick so we can cancel the 'tap' event if
					// 'scroll' fires
					...
					tapTimeout = setTimeout(function() {
								...
								// cancelTouch()
								// (cancelTouch cancels processing of single vs
								// double taps for faster 'tap' response)
								var event = $.Event('tap')
	...
	['swipe', 'swipeLeft', 'swipeRight', 'swipeUp', 'swipeDown', 'doubleTap',
			'tap', 'singleTap', 'longTap'].forEach(function(m) {
...
window.utils = (function() {
	return {
		...
		getRemainedDays : function(b, e, a) {
			var c = new Date(b, e-1, a, 23, 59, 59);
			var f = Math.floor((c.getTime() - (new Date().getTime()))
					/ (1000 * 60 * 60 * 24));
			return f < 0 ? 0 : f
		},
		...
		reposition : function(ele) {
			...
			ele.css({
						position : "fixed",
						top : Math.abs(this.getClientSize().height - Number(h))
								/ 2 + "px",
						left : (this.getClientSize().width - Number(w)) / 2
								+ "px"
					});
		},
		initSlide : function() {
			var that = this, img_w = $("#slide_wrap").width(), // ,$(".list").width()/4,
...
(function(a) {
	a.fn.vslide = function(b) {
		...
		this.each(function() {
					var that = this;
					var h = that, e = a(h).width(), m = a(h).height(), l = a(h)
							.find("li").eq(0).width(), i = a(h).find("ul"), k = l
					...
					if (b.left_bt) {
						a(".btn1").tap(function() {
							...
							a(this).closest(b.box).find(b.right_bt).css(
									"opacity", "0.5");
							j("left", i)
						})
					}
					if (b.right_bt) {
						a(".btn2").tap(function() {
							...
							a(this).closest(b.box).find(b.left_bt).css(
									"opacity", "0.5");
							j("right", i)
						})
					}
					...
					a(window).on("orientationchange", function() {
								...
								e = a(that).width();
								m = a(that).height();
								l = a(that).find("li").eq(0).width();
								i = a(that).find("ul");
								k = l * a(that).find("li").length;
								f = 0;
								i.css({
											width : k,
											marginLeft : 0
